...
namespace HoloToolkit.Unity.InputModule
{
    ...
    public partial class KeywordManager : MonoBehaviour
    {
        ...
        public Palettierer_Actions rootAction;
        ...
        void Start()
        {
            ...
            if (keywordCount > 0)
            {
                try
                {
                    string[] keywords = new string[keywordCount + 36];
                    // Convert the struct array into a dictionary, with the keywords and the keys and the methods as the values.
                    // This helps easily link the keyword recognized to the UnityEvent to be invoked.
                    int keyWordIndex = 0;
                    for (; keyWordIndex < keywordCount; keyWordIndex++)
                    {
                        KeywordAndResponse keywordAndResponse = KeywordsAndResponses[keyWordIndex];
                        responses[keywordAndResponse.Keyword] = keywordAndResponse.Response;
                        keywords[keyWordIndex] = keywordAndResponse.Keyword;
                    }
                    
                    for (int i = 0; i < 360; i+=10)
                    {
                        string keyword = "Rotate " + i;
        ...
        private void KeywordRecognizer_OnPhraseRecognized(PhraseRecognizedEventArgs args)
        {
            ...
            if (args.text.StartsWith("Rotate"))
            {
                try
                {
                    int degrees = Int32.Parse(args.text.Substring(7));
                    Debug.Log("Degrees " + degrees);
                    rootAction.Rotate(degrees);
                } catch (Exception e)
                {
                    Debug.LogError(e);
                }
